...
async def create_user_profile(new_tg_id):
    ...
    async with async_session() as session:
        try:
            existing = await session.scalar(select(User).where(User.tg_id == new_tg_id)) # проверка на существование пользователя
...
async def remove_category(category_id: int) -> bool:
    async with async_session() as session:
        ...
        await session.execute(
            delete(Product).where(Product.category_id == category_id)
        )
...
async def products_setter(new_product, new_price, new_category_id):
    async with async_session() as session:
        session.add(Product(name = new_product, price = new_price, category_id = new_category_id))
        await session.commit()
...
async def add_to_cart(product_id: int, user_id: int, quantity = int) -> bool:
    async with async_session() as session:
        ...
        session.add(Cart(product_id = product_id, user_id = user_id, quantity = quantity, added_price = added_price, product_name = added_name))
        await session.commit()
